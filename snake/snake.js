...
function changeDirection(orientation) {
    //make orientation a pointer on one of the two arrays
    //whatever you do, you raise the pointer of either array

    const currentPosition = orientation.indexOf(direction); //it is at the position/index where north is in the arrays
    direction = orientation[currentPosition + 1];
...
function nextBoard() {
    ...
    if (snakeEquals(food, head)) {  // have we found any food?
        ...
        food.y = Math.floor(Math.random() * 20);
          ...
          snake.unshift(head);// put head at front of the snake
